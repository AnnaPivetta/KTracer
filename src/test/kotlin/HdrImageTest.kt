...
class HdrImageTest {
    //Content of "reference_le.pfm" (little-endian file)
/*    val LE_RAW_BYTES = arrayOf(
    ...
    val LE_REFERENCE_BYTES = ByteArray({for (i in LE_RAW_BYTES.indices) {LE_RAW_BYTES[i].toByte()}})

    //Content of "reference_be.pfm" (big-endian file)
    ...
    val BE_REFERENCE_BYTES = ByteArray({i:Int -> BE_RAW_BYTES[i].toByte()})
*/
    @Test
    ...
    fun readPfmFile() {
        ...
        val LE_REFERENCE_BYTES = ByteArray(LE_RAW_BYTES.size)
        for (i in LE_RAW_BYTES.indices) LE_REFERENCE_BYTES[i] = LE_RAW_BYTES[i].toByte()
        ...
        val BE_REFERENCE_BYTES = ByteArray(BE_RAW_BYTES.size)
        for (i in BE_RAW_BYTES.indices) BE_REFERENCE_BYTES[i] = BE_RAW_BYTES[i].toByte()
